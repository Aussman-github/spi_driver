...
pr_info("        xfer: tx_buf=%p, rx_buf=%p, len=%d\n", xfer->tx_buf, xfer->rx_buf, xfer->len);
...
//				       GFP_KERNEL | GFP_DMA);
if (master->dummy_tx) {
pr_info("    dummy_tx: krealloc(%p, %u)\n", master->dummy_tx, max_tx);
	tmp = krealloc(master->dummy_tx, max_tx,
                       GFP_KERNEL | GFP_DMA);
pr_info("    dummy_tx: krealloc returned %p\n", tmp);
} else {
pr_info("    dummy_tx: kmalloc(%u)\n", max_tx);
	tmp = kmalloc(max_tx,
	               GFP_KERNEL | GFP_DMA);
pr_info("    dummy_tx: kmalloc returned %p\n", tmp);
...
if (master->dummy_rx) {
pr_info("    dummy_rx: krealloc(%p, %u)\n", master->dummy_rx, max_rx);
	tmp = krealloc(master->dummy_rx, max_rx,
                       GFP_KERNEL | GFP_DMA);
pr_info("    dummy_rx: krealloc returned %p\n", tmp);
} else {
pr_info("    dummy_rx: kmalloc(%u)\n", max_rx);
	tmp = kmalloc(max_rx,
	               GFP_KERNEL | GFP_DMA);
pr_info("    dummy_rx: kmalloc returned %p\n", tmp);
